...
def main():
    ...
    rel_lines = plt.vlines(bug_dates, 0, max(normal_counts), color='#616D7E', linestyles='dashed', label='_nolegend_')
    ...
    ax.set_ylim(0, max(normal_counts)+30)
    add_metadata(ax,corr)
    add_label(bug_dates,normal_counts, bug_descr)
    plt.show()
    #export()
...
def add_trend(x, y):
    ...
    slope, intercept = z
    p = np.poly1d(z)
    trend_line = plt.plot(x, p(new_x), 'k-', label='\^{{y}} = {0:.2} + {1:.2%}'.format(intercept,slope))
    #plt.text()
    return corr
    
def add_label(bug_dates, normal_counts, bug_descr):
