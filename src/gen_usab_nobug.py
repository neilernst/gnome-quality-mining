...
#datetime.datetime(1997, 8, 1),'0.0', 
    ...
    for dateweek in date_sort:
        ...
        dates.append(datetime.datetime.strptime(dateweek, '%Y%U-%w') )


    plt.plot(dates, x.values(),'r-', antialiased=True, label='Occurrences') #  problem is the dates are not monotonically increasing
    rel_lines = plt.vlines(bug_dates, 0, max(x.values()), color='#616D7E', linestyles='dashed', label='_nolegend_')
    ...
    #corr, slope, intercept = add_trend(dates, x.values())

    ax.set_xlim(datemin, datemax)
    ax.set_ylim(0, max(x.values())+0.13)
    r2 = add_metadata()#(ax,corr)
    add_label(dates, bug_dates, x.values(), bug_descr)
    plt.show()
...
def add_metadata():#(ax,corr):
    ...
    plt.xlabel("Date")
    plt.title("Frequency of signifier occurrence over time\n" + 'Product: Nautilus, signifier: Usability', verticalalignment='bottom')
