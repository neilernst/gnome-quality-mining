...
\caption{Defining signified terms extensionally}
  \label{alg1}
\begin{algorithmic}
  \FORALL{top level terms in ISO9126}
	\STATE $S \rightarrow []$
    \STATE define it as a wordnet noun
\STATE query its synset in wordnet
\STATE identify its direct hypernyms (sub-parts)
\STATE do query for each member of S
	\COMMENT ignore case
  \ENDFOR
...
\end{algorithmic}
\end{algorithm}
Note, we do not currently look for common misspellings or other languages (the primary language is English, at any rate).
...
This algorithm gives us a linguistic `bubble' which we use to extensionally define the signified term.
